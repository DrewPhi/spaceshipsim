...
from .models import DossierEntity, DossierFact, DossierQuestion, LoreExpansion, SituationDossier
...
class UniverseArchitect:
    ...
    async def expand_for_question(
        ...
        if not question or self.canon.question_already_expanded(question):
            return None
        request = {
            ...
            "current_situation": self.canon.director_context(encounter.id, limit=50),
        ...
        try:
            expansion = LoreExpansion.model_validate({**payload, "question": payload.get("question") or question})
